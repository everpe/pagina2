...
var textos=[''];
let contador=0;//contador usado para recorrer los arreglos de texto e imagen del Slider.
...
function menus(){
    ...
    if(desplazamiento_Actual<=350){//muestra Nav1
        ...
        menu.style.top='85px';
        abrir.style.color='rgb(54, 113, 131)'; 
        // alert('llegué');
 ...
 */
function slider(){
    let desplazamiento_Actu=window.pageYOffset;
    if(screen.width<=700)
    {
        if(desplazamiento_Actu<=1080&&cerrado){
            $(flecha).show();
            $(flecha2).show();
        }else{
            $(flecha).hide();
        ...
        if(desplazamiento_Actu<=1000&&cerrado){
            $(flecha).show();
            $(flecha2).show();
        }else{
            $(flecha).hide();
...
function carrousel (e){
    ...
    let h33=conte.querySelector('h2');//titulo de la imagen que cambia del Slider 

    if(e.target == flecha){
        // alert(contador);
        img.src=galeria[contador];
        h33.textContent=nombres[contador];
        contador=contador-2;
        if(contador<0){
